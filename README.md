...
The concepts in this lab extend on concepts covered in RD00. Before starting this lab you should complete the reading and the guided reading questions.

## Setup
Before you begin programming in each part, carefully read the section and complete the corresponding guided reading questions on Gradescope. Once you’ve answered those questions, you may begin implementing the code.

**So, before carrying out the setup steps below, read through the section and complete its guided reading questions on Gradescope first!**
...
### VS Code Setup
You should have already completed the following steps in HW00. If you did not, they are here:

1. Download [VSCode](https://code.visualstudio.com/download).

2. Install the [dev container extension](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-containers).

3. In VSCode, open the `learncli211` project folder (do not open a subfolder of `learncli211`!).

4. Click the remote status bar at the bottom left, then click **reopen in container**.

5. To open a terminal, go to the top menu bar and select **Terminal > New Terminal**.


## Part 0. Hello, world!
**Before carrying out the steps below, read through the section and complete its guided reading questions on Gradescope first!**

In the terminal window, navigate to your `Lab00` directory using the following command:

```
$ cd <lab00-yourgithubusername>
```

Inside your Lab00 directory, make a new directory named exactly `0-hello-world`. Change your working directory to be this new subdirectory. For reference on how to make a new directory or change directories, please refer to [_Chapter 1: The Sorcerer's Shell_](https://uncch.instructure.com/courses/95468/files/folder/readings?preview=12214321).
...
1. Your program should print "hello, world" on one line and, "welcome to c!" on another line.
2. `main` should return `EXIT_SUCCESS`. To return `EXIT_SUCCESS` you will need to import `stdlib.h`, the header file which defines this constant. When `main` returns `EXIT_SUCCESS` it indicates the program completed successfully via a success _exit status_. We will explore the idea of _exit statuses_ later this semester. 
3. Additionally, in 211 we expect all function return types to be defined and `main` should return an `int`.
...
~~~bash
$ gcc -Wall -Wextra -Wpedantic hello.c
~~~
...
Start by navigating back to the base directory of `Lab 00`, which is the parent directory of `0-hello-world`.
Use the `cd` command to move to the appropriate directory.
...
Begin your work by creating a new file named `strip-comments.c` in the `1-bash-comments` directory.

Your program should process input character-by-character using `getchar` and use states to track whether it is inside a comment or not.

Remember to return the proper exit status (`EXIT_SUCCESS` or `EXIT_FAILURE`).

### Testing

There are three test inputs provided for you: `00-verbatim.sh`m `01-comments.sh`, and `02-strings.sh`. These test scripts are written in order of difficulty. You are encouraged to get your program to match the first file's expected results before moving to the next.
...
```
$ cat 00-verbatim.sh | ./a.out
```

The above command sends the contents of the file `00-verbatim.sh` as input to the program `./a.out`. Here's a step-by-step explanation of how this works:

1. The `cat` command prints the contents of `00-verbatim.sh` to standard output (your terminal screen by default).

2. The pipe operator (`|`) takes the output from the command on the left (in this case, the text printed by `cat`) and redirects it as input to the command on the right (here, the program `./a.out`). 
    - The contents of `00-verbatim.sh` are not printed to the terminal because they are being redirected to the program input.

3. When you run your program (`./a.out`), it reads from standard input. In this case, the input is not typed manually; instead, the pipe sends the contents of `00-verbatim.sh` directly to the program.
...
Here, `gcc` referes to the GNU Compiler Collection (GCC), the compiler that translates your C source code into an executable program.

**args**

These are the argumengs given to `gcc` that tell it how to compile your file:
...
```
$ gcc -g -Wall -Wextra -Wpedantic strip-comments.c -o strip-comments
```
...
In this case, the debugger will pipe the contents of the file `00-verbatim.sh` into the program’s standard input.

#### Steps to Debug
**Setting up the config files.**

These steps were done in class on Friday, August 29, 2025. They are here for your reference.

1. Inside of `workdir`, make a new directory called `.vscode`.
2. Move the provided `launch.json` and `tasks.json` files to the `.vscode` directory.

**Debugging**

Debug just as we have done in class (see slides posted on Canvas). When you would like to debug other inputs, edit `launch.json` and replace `00-verbatim.sh` with the desired file.

### Submiting to Gradecope
...
To begin, change your working directory to be back at Lab 00's base, as in the parent directory of `0-hello-world`. Then, make a new directory named `2-char-count`, and in that directory create a file named `char-count.c`

### 2.1 - Counts

Your program should count occurrences of alphabetical, English letters `a` through `z`, case insensitive. It should ignore all other characters except for the `EOF` character. When the `EOF` character is encountered, your program should display the counts of _only_ the alphabetical characters with at least one occurrence. The expected format of this output is, for each character and count on its own line: `<char>: <count>`.
...
#### Testing
To test your program, think of different input strings that cover the various edge cases and ensure that your program handles them properly. For example, try a string with a combination of letters from the English alphabet and special characters, then try a string with only special characters, etc.

**It is strongly recommended to fully implement and ensure that Counts is functioning correctly before proceeding to section 2.2.**

### 2.2 - Percents

After outputing character counts, your program should output each character's percentage of the alphabetical characters encountered. For example, in the example input "Hello, World!" there were were 10 alphabetical characters and 3 of them were the letter `l`, so `l`'s percentage was `30.0%`. The expected format of this output is, for each character and percentage on its own line, each percentage should have _one digit of significance_ and be followed by the `%` symbol: `<char>: <percent>%`.
...
**It is strongly recommended to fully implement and ensure that Percents is functioning correctly before proceeding to section 2.3.**
...
For example, in the example input "Hello, World!", `l` is the most frequently occuring character, so `l`'s bar will be 10 lines tall. `o` appears twice, while `l` appears 3 times, so `o`'s bar will be 2 / 3 * 10 = 6.67 -> 6 lines tall. Refer back to the example at the top of this section to see the full chart.
...
(5 points) Style: manually graded by course staff, after the late deadline, on the following criteria:
 ...
 - All magic numbers are defined as constants (0, 1, and 100.0 are acceptable number literals to use in code)
    - See section 1.4 of the textbook
